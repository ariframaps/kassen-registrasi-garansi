...
import { mockDashboardStats, mockRecentActivity, mockSalesNotifications } from "@/lib/mock-data";
import { useAuth } from "@/lib/auth-context";
import { Package, Layers, Shield, ShieldOff, Clock, TrendingUp, Bell, Circle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  return (
    <div>
      <Topbar title="Dashboard" description={isSales ? `Selamat datang, ${user?.name}` : "Ringkasan sistem registrasi garansi"} />
        ...
        {isSales ? (
          ...
          <Card className="animate-fade-up delay-2">
            <CardHeader
              ...
              action={
                <Badge variant="warning">{mockSalesNotifications.filter(n => !n.read).length} baru</Badge>
              }
            />
            <CardContent className="p-0">
              <ul className="divide-y divide-zinc-50">
                {mockSalesNotifications.map((n, i) => (
                  <li key={n.id} className={`flex items-start gap-3 px-5 py-3.5 hover:bg-zinc-50/50 transition-colors animate-fade-up delay-${Math.min(i + 1, 5)} ${!n.read ? "bg-amber-50/40" : ""}`}>
                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${!n.read ? "bg-amber-100" : "bg-zinc-100"}`}>
                      <Clock size={13} className={!n.read ? "text-amber-600" : "text-zinc-400"} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`text-xs ${!n.read ? "text-zinc-900 font-medium" : "text-zinc-600"}`}>{n.description}</p>
                        {!n.read && <Circle size={6} className="text-amber-500 fill-amber-500 shrink-0" />}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] text-zinc-400">{n.email}</span>
              ...
              <div className="px-5 py-3 border-t border-zinc-50">
                <a href="/dashboard/waiting-list" className="text-xs text-blue-600 hover:underline">Lihat semua di Waiting List →</a>
              </div>
            </CardContent>
          </Card>
        ) : (
          ...
          <Card className="animate-fade-up delay-2">
            <CardHeader title="Aktivitas Terbaru" description="Log semua aktivitas sistem dalam 7 hari terakhir" />
            <CardContent className="p-0">
              <ul className="divide-y divide-zinc-50">
