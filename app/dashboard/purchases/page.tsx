	...
	const available = mockProducts.filter(
		(p) =>
			...
			(newSn === "" ||
				...
				p.productType.toLowerCase().includes(newSn.toLowerCase())),
	);
		...
		<>
			<div className="p-3 border border-blue-200 bg-blue-50/50 rounded-xl space-y-3">
				...
				<div className="space-y-1">
					{sns.map((sn) => {
						const p = mockProducts.find((x) => x.serialNumber === sn);
						return (
							<div
								key={sn}
			...
			<div className="p-6 animate-fade-up">
				...
				<div className="grid grid-cols-4 gap-4 mb-5">
					{[
						{
							l: "Total Pembelian",
							v: mockPurchaseGroups.length,
							c: "text-zinc-900",
						},
						{
							l: "Via Dealer",
							v: mockPurchaseGroups.filter((g) => g.dealerId).length,
							c: "text-blue-700",
						},
						{
							l: "Via Sales",
				...
				<Card>
					...
					<div className="px-5 py-3.5 border-b border-zinc-100 flex flex-wrap gap-2.5 items-center">
						<div className="flex-1 min-w-48 max-w-64">
							<Input
								placeholder="Cari customer, SN, atau dealer…"
								value={search}
												...
												<TableRow key={g.id}>
													...
													<TableCell>
														<span className="text-xs text-zinc-500">
															{g.dealerName ?? g.registeredByName ?? "—"}
														</span>
													</TableCell>
													<TableCell>
														<Badge
							...
							<div className="space-y-5">
								...
								<div className="flex items-start gap-3 p-4 bg-zinc-50 rounded-xl border border-zinc-100">
									...
									<div className="flex-1">
										<div className="flex items-center gap-2 mb-0.5">
											...
											<span className="text-xs text-zinc-400">·</span>
											<span className="text-xs text-zinc-500">
												{formatDateShort(selected.registeredAt)}
											</span>
										</div>
								...
								<div className="grid grid-cols-3 gap-4">
									...
									<div className="p-3 bg-zinc-50 rounded-xl">
										...
										<p className="text-xs font-semibold text-zinc-800">
											{selected.serialNumbers.length} unit
										</p>
									</div>
									<div className="p-3 bg-zinc-50 rounded-xl">
										<div className="flex items-center gap-1.5 mb-1">
										...
										<p className="text-xs font-semibold text-zinc-800">
											{selected.dealerName ?? selected.registeredByName ?? "—"}
										</p>
									</div>
								</div>
