...
import { getProductStatusLabel, getProductStatusBadgeVariant, formatDateShort } from "@/lib/utils";
import { Search, Eye, Shield, Package, X, Check, Link2, ChevronDown } from "lucide-react";
...
function SalesWarrantyModal({
...
}: { open: boolean; products: Product[]; onClose: () => void }) {
  const [form, setForm] = useState({ customerName: "", phone: "", email: "", warrantyStart: "", invoiceFile: null as File | null });
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    onClose();
    success("Garansi berhasil diregistrasikan", `${products.length} produk terdaftar`);
  };

  const groupId = form.customerName && form.warrantyStart
    ? `GRP-${form.customerName.slice(0, 3).toUpperCase()}-${form.warrantyStart.replace(/-/g, "").slice(2)}`
    : null;

  return (
    <Modal open={open} onClose={onClose} title="Registrasi Garansi — Sales" description={`${products.length} produk dipilih`} size="md">
      {/* Selected SNs */}
      <div className="p-3 bg-zinc-50 border border-zinc-100 rounded-xl mb-4">
        <div className="flex flex-wrap gap-1.5 mb-2">
          {products.map(p => (
            <span key={p.id} className="font-mono text-[11px] bg-white border border-zinc-200 px-2 py-0.5 rounded-md text-zinc-600">
              {p.serialNumber}
            </span>
          ))}
        </div>
        <p className="text-[11px] text-amber-600">⚠ Semua produk menggunakan 1 data customer & 1 invoice</p>
        {groupId && (
      ...
      <div className="space-y-3">
        <Input label="Nama Customer" placeholder="Nama lengkap" required value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })} error={errors.customerName} />
        ...
        <Input label="Tanggal Terjual (= Mulai Garansi)" type="date" required value={form.warrantyStart} onChange={e => setForm({ ...form, warrantyStart: e.target.value })} error={errors.warrantyStart} hint="Tanggal ini menjadi tanggal mulai garansi" />
        <div>
          <label className="block text-xs font-medium text-zinc-700 mb-1.5">Invoice <span className="text-red-500">*</span></label>
          <label className={`flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${form.invoiceFile ? "border-emerald-400 bg-emerald-50" : "border-zinc-200 hover:border-zinc-300 bg-zinc-50"}`}>
            {form.invoiceFile ? (
              <div className="flex items-center gap-2 text-emerald-700">
                <Check size={14} />
                <span className="text-xs font-medium">{form.invoiceFile.name}</span>
                <button type="button" onClick={e => { e.preventDefault(); setForm({ ...form, invoiceFile: null }); }} className="text-zinc-400 hover:text-zinc-600 ml-1"><X size={11} /></button>
              </div>
            ) : (
              <p className="text-xs text-zinc-400">Klik untuk upload — PNG, JPG, PDF · Maks 4MB</p>
            )}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <Button fullWidth loading={loading} icon={<Shield size={13} />} onClick={handleSubmit}>Registrasikan Garansi</Button>
        </div>
      </div>
    </Modal>
  );
...
function BulkAssignModal({
  ...
  const assignable = mockProducts.filter(p =>
  ...
  const handleAssign = async () => {
    ...
    onClose();
    setSelectedSNs([]); setDealerId(""); setSearch("");
    success(`${selectedSNs.length} produk berhasil di-assign`, `ke ${mockDealers.find(d => d.id === dealerId)?.name}`);
  };

  return (
    <>
      <Modal open={open} onClose={onClose} title="Assign Produk ke Dealer" description="Pilih produk dan dealer tujuan" size="xl">
        <div className="space-y-4">
          {/* Dealer selector */}
          <Select
            label="Dealer Tujuan"
            required
            options={mockDealers.filter(d => d.status === "active").map(d => ({ value: d.id, label: d.name }))}
          ...
          <div className="flex gap-2 items-center">
            ...
            {selectedSNs.length > 0 && (
              <button onClick={() => setSelectedSNs([])} className="text-xs text-zinc-400 hover:text-zinc-600 whitespace-nowrap px-1">
                Batal Semua
              </button>
            )}
          </div>
          ...
          <div className="border border-zinc-100 rounded-xl overflow-hidden">
            <div className="max-h-64 overflow-y-auto divide-y divide-zinc-50">
              {assignable.length === 0 ? (
                <div className="py-8 text-center text-xs text-zinc-400">Tidak ada produk yang belum di-assign</div>
              ) : assignable.map(p => {
                ...
                return (
                  ...
                  >
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${sel ? "bg-blue-600 border-blue-600" : "border-zinc-300"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-mono text-xs font-semibold text-zinc-800">{p.serialNumber}</p>
                      <p className="text-[11px] text-zinc-400 truncate">{p.productType} · {p.productCategory}</p>
                    </div>
                  </button>
                );
              })}
          ...
          {selectedSNs.length > 0 && (
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl">
              <p className="text-xs font-medium text-blue-700 mb-2">{selectedSNs.length} produk dipilih</p>
              <div className="flex flex-wrap gap-1.5">
                {selectedSNs.map(sn => (
                  <span key={sn} className="inline-flex items-center gap-1 bg-white border border-blue-200 px-2 py-0.5 rounded-md text-[11px] font-mono text-blue-700">
                    {sn}
                    <button onClick={() => toggleSN(sn)}><X size={9} /></button>
                  </span>
                ))}
              </div>
            </div>
...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return mockProducts.filter(p => {
      ...
      const matchDealer   = dealerFilter === "all"   || (dealerFilter === "none" ? !p.assignedDealerId : p.assignedDealerId === dealerFilter);
  ...
  const toggleWarranty = (id: string) =>
    setWarrantySelected(prev => prev.includes(id) ? prev.filter(s => s !== id) : [...prev, id]);

  const canSalesRegister = (p: Product) => !p.assignedDealerId && p.warrantyStatus === "none";
  const warrantySelectedProducts = mockProducts.filter(p => warrantySelected.includes(p.id));
  ...
  return (
    <div>
        ...
        <Card>
                  ...
                  {warrantySelectMode ? (
                    <>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => { setWarrantySelectMode(false); setWarrantySelected([]); }}
                      >
                        Batal
                      </Button>
                      <Button
                    ...
                    <>
                      <Button size="sm" variant="outline" icon={<Link2 size={13} />} onClick={() => setBulkAssignOpen(true)}>
                        Assign ke Dealer
                      </Button>
                      <Button size="sm" icon={<Shield size={13} />} onClick={() => setWarrantySelectMode(true)}>
                        Registrasi Garansi
                      </Button>
                    </>
                  )}
          ...
          {warrantySelectMode && (
            <div className="px-5 py-2.5 bg-blue-50 border-b border-blue-100 flex items-center gap-2">
              ...
              <p className="text-xs text-blue-700">
                Mode registrasi — pilih produk yang <strong>belum di-assign ke dealer</strong> dan belum punya garansi aktif
              </p>
            </div>
          )}
          ...
          <CardContent className="p-0">
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={warrantySelectMode ? 9 : 8}>
                      <EmptyState icon={<Package size={18} />} title="Tidak ada produk" description="Ubah filter atau kata kunci pencarian" />
                    </td>
                  </tr>
                ) : filtered.map(p => {
                  const selectable = warrantySelectMode && canSalesRegister(p);
                  const sel = warrantySelected.includes(p.id);
                  return (
                    <TableRow
                      key={p.id}
                      className={warrantySelectMode && selectable ? "cursor-pointer" : ""}
                      onClick={warrantySelectMode && selectable ? () => toggleWarranty(p.id) : undefined}
                    >
                      {warrantySelectMode && (
                        <TableCell>
                          {selectable ? (
                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${sel ? "bg-blue-600 border-blue-600" : "border-zinc-300"}`}>
                              {sel && <Check size={9} className="text-white" strokeWidth={3} />}
                            </div>
                          ) : (
                            <div className="w-4 h-4 rounded border-2 border-zinc-100 bg-zinc-50" title="Produk sudah di dealer atau sudah punya garansi" />
                          )}
                        </TableCell>
                      )}
                      <TableCell>
                      ...
                      <TableCell>
                        <span className="text-xs text-zinc-900">{p.productType}</span>
                      </TableCell>
                      <TableCell>
                        <span className="text-xs text-zinc-500">{p.productCategory}</span>
                      </TableCell>
                      <TableCell>
                        <Badge variant={getProductStatusBadgeVariant(p.status)} dot>
                          {getProductStatusLabel(p.status)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <span className="text-xs text-zinc-500">{p.assignedDealerName ?? <span className="text-zinc-300">—</span>}</span>
                      </TableCell>
                      <TableCell>
            ...
            <div className="px-5 py-3 border-t border-zinc-50 flex items-center justify-between">
              <p className="text-xs text-zinc-400">Menampilkan {filtered.length} dari {mockProducts.length} produk</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Detail Modal */}
      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title="Detail Produk" size="md">
        {selectedProduct && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-mono text-sm font-semibold text-zinc-900">{selectedProduct.serialNumber}</p>
                <p className="text-xs text-zinc-500">{selectedProduct.productType} · {selectedProduct.productCategory}</p>
              </div>
              <Badge variant={getProductStatusBadgeVariant(selectedProduct.status)} dot>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {[
                ...
                { l: "Mulai Garansi",   v: selectedProduct.warrantyStartDate ? formatDateShort(selectedProduct.warrantyStartDate) : "—" },
                { l: "Berakhir Garansi",v: selectedProduct.warrantyEndDate ? formatDateShort(selectedProduct.warrantyEndDate) : "—" },
              ].map(item => (
                <div key={item.l}>
                  <p className="text-[11px] text-zinc-400">{item.l}</p>
                  <p className="text-xs font-medium text-zinc-800 mt-0.5">{item.v}</p>
                </div>
              ))}
            </div>
      ...
      {/* Bulk Assign */}
      <BulkAssignModal open={bulkAssignOpen} onClose={() => setBulkAssignOpen(false)} />

      {/* Sales Warranty Registration */}
      <SalesWarrantyModal
        open={warrantyModalOpen}
        products={warrantySelectedProducts}
        onClose={() => { setWarrantyModalOpen(false); setWarrantySelectMode(false); setWarrantySelected([]); }}
      />
    </div>
  );
}
