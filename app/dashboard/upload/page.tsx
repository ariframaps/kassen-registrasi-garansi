...
const MOCK_PREVIEW: PreviewRow[] = [
	...
	},
	{
		...
		productCategory: "POS System",
		status: "duplicate",
		message: "Serial number sudah ada di sistem",
	},
	{
...
export default function UploadPage() {
	...
	const handleUpload = async () => {
		...
		success(
			"Upload berhasil",
			`${validCount} produk baru ditambahkan ke sistem`,
		);
	};
			...
			<div className="p-6 animate-fade-up space-y-5 max-w-3xl">
				...
				<div className="grid grid-cols-2 gap-4">
					{[
						{
							n: "1",
							t: "Download File dari Accurate",
							d: "Export file Excel yang tersedia di Accurate",
						},
						{
							n: "2",
							t: "Upload & Konfirmasi",
							d: "Upload, preview hasilnya, lalu konfirmasi",
						},
					].map((s) => (
						<div
							key={s.n}
				...
				<Card>
					<CardHeader title="Upload File Excel" />
					<CardContent>
						<div
							onDragOver={(e) => {
								e.preventDefault();
						...
						<CardHeader
							...
							action={
								<div className="flex items-center gap-1.5">
									...
									{dupCount > 0 && (
										<Badge variant="warning">{dupCount} duplikat</Badge>
									)}
									{invCount > 0 && (
										<Badge variant="danger">{invCount} invalid</Badge>
									)}
								</div>
							}
						/>
							...
							<div className="px-5 py-4 border-t border-zinc-50">
								{validCount === 0 ? (
									<div className="flex items-center gap-2 text-xs text-red-600">
										...
										<span>
											Tidak ada data valid. Periksa kembali file Anda.
										</span>
									</div>
								) : (
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-2 text-xs text-emerald-700">
											<CheckCircle size={14} />
											<span>
												<strong>{validCount} produk</strong> siap diupload ke
												sistem
											</span>
										</div>
										<Button
