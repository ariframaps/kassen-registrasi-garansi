...
// ── Item Code Tag ──
function ItemCodeTag({ code, onRemove }: { code: string; onRemove?: () => void }) {
  return (
...
function ProductTypeModal({
  ...
  const [itemCodes, setItemCodes] = useState<string[]>(initial?.itemCodes ?? []);
  ...
  const addCode = async () => {
    ...
    if (existing && existing.id !== initial?.id) {
      setDuplicateError(`Item code '${code}' sudah digunakan oleh tipe "${existing.name}".`);
      return;
    }
  ...
  const removeCode = (code: string) => setItemCodes((prev) => prev.filter((c) => c !== code));
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            ...
            />
            <Button variant="outline" size="sm" onClick={addCode} icon={<Plus size={13} />}>
              Tambah
            </Button>
          </div>
          ...
          {itemCodes.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">
              {itemCodes.map((code) => (
                <ItemCodeTag key={code} code={code} onRemove={() => removeCode(code)} />
              ))}
            </div>
          ) : (
            <p className="text-xs text-zinc-400">Belum ada item code. Item code boleh dikosongkan.</p>
          )}
        </div>
      ...
      <main className="flex-1 p-6 space-y-5 animate-fade-up">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((cat) => {
            ...
            return (
              <div key={cat.id} className="bg-white border border-zinc-200 rounded-xl px-4 py-3 shadow-sm">
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={5}>
                      <EmptyState
                        ...
                        description={search ? "Coba ubah kata kunci pencarian" : "Tambahkan tipe produk pertama Anda"}
                      />
                    </td>
                  </tr>
                ) : (
                  filtered.map((type) => (
                    <TableRow key={type.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {type.itemCodes.length === 0 ? (
                            <span className="text-xs text-zinc-400 italic">Belum ada mapping</span>
                          ) : (
                      ...
                      <TableCell className="text-zinc-500 text-xs">{type.createdAt}</TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <button
                            onClick={() => { setEditing(type); setModalOpen(true); }}
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4">
          <div className="flex gap-2.5 items-start">
            ...
            <div>
              <p className="text-xs font-medium text-blue-800 mb-1">Cara kerja mapping item code</p>
              <p className="text-xs text-blue-700 leading-relaxed">
                Saat upload file DO dari Accurate, sistem akan mencocokkan <code className="bg-blue-100 px-1 rounded">item_code</code> di file dengan daftar mapping di sini.
                Jika ditemukan, nama tipe yang benar akan otomatis digunakan. Jika tidak ditemukan, sistem akan menampilkan peringatan dan Anda bisa menambahkan mapping baru langsung dari halaman upload.
              </p>
            </div>
          </div>
        </div>
