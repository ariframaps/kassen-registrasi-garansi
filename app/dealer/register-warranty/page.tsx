...
export default function RegisterWarrantyPage() {
	...
	const toggle = (sn: string) =>
		setForm((p) => ({
			...p,
			selectedSNs: p.selectedSNs.includes(sn)
				? p.selectedSNs.filter((s) => s !== sn)
				: [...p.selectedSNs, sn],
		}));
	...
	const groupId =
		form.customerName && form.purchaseDate
			...
			<div className="p-6 max-w-xl mx-auto">
					...
					{[
						...
						<div key={s.n} className="flex items-center flex-1">
							<button
								...
								<span
									className={`text-xs font-medium ${step === s.n ? "text-zinc-900" : "text-zinc-400"}`}>
									{s.l}
								</span>
							</button>
							{i < 1 && (
								<div
									className={`flex-1 h-px mx-3 ${step > s.n ? "bg-emerald-400" : "bg-zinc-200"}`}
								/>
							)}
						</div>
					))}
				...
				{step === 1 && (
					<Card className="animate-fade-up">
						<CardHeader
							title="Pilih Produk"
							description="Pilih produk yang akan didaftarkan garansinya"
						/>
						<CardContent>
							{/* Search */}
							<div className="mb-3">
									...
									{filteredProducts.map((p) => {
										...
										return (
											<button
												...
												<div
													className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${sel ? "bg-blue-600 border-blue-600" : "border-zinc-300"}`}>
													{sel && (
														<Check
															size={10}
							...
							{form.selectedSNs.length > 0 && (
								<div className="mt-3 p-3 rounded-xl bg-blue-50 border border-blue-100">
									<p className="text-xs text-blue-700 font-medium mb-2">
										{form.selectedSNs.length} dipilih
									</p>
				...
				{step === 2 && (
					<Card className="animate-fade-up">
						<CardHeader
							title="Data Customer"
							description={`Untuk ${form.selectedSNs.length} produk terpilih`}
						/>
						<CardContent>
							...
							<div className="p-3 bg-zinc-50 border border-zinc-100 rounded-xl mb-4">
								...
								<p className="text-[11px] text-amber-600">
									⚠ Semua produk di atas menggunakan 1 data customer & 1 invoice
							...
							<div className="space-y-3">
								...
								<div>
									<label className="block text-xs font-medium text-zinc-700 mb-1.5">
										Foto Invoice <span className="text-red-500">*</span>
									</label>
									<label
										className={`flex flex-col items-center justify-center p-5 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${form.invoiceFile ? "border-emerald-400 bg-emerald-50" : "border-zinc-200 hover:border-zinc-300 bg-zinc-50"}`}>
										{form.invoiceFile ? (
											...
											<div className="text-center">
												<Upload
													size={20}
													className="text-zinc-300 mx-auto mb-1.5"
												/>
												<p className="text-xs text-zinc-400">
													Klik untuk upload
												</p>
												<p className="text-[11px] text-zinc-300 mt-0.5">
													PNG, JPG, PDF · maks 4MB
												</p>
											</div>
										)}
										<input
			...
			<Modal
				...
				<div className="text-center py-3">
					...
					<h3 className="text-base font-semibold text-zinc-900 mb-1.5">
						Garansi Berhasil Terdaftar
					</h3>
					<p className="text-xs text-zinc-500 mb-1">
						<span className="font-semibold text-zinc-800">
							{form.selectedSNs.length} produk
						</span>{" "}
						atas nama{" "}
						<span className="font-semibold text-zinc-800">
							{form.customerName}
