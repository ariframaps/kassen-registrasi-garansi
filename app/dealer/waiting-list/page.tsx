...
const dealerRequests = mockWaitingList.filter(
	(w) => w.requestorType === "dealer" && w.dealerId === "d1",
);
...
export default function DealerWaitingListPage() {
	...
	const handleRequest = async () => {
		...
		success(
			...
			`SN ${normalizeSerialNumber(serialInput)} masuk ke waiting list.`,
		);
	};
	...
	return (
		<div>
			<Topbar
				...
				description="Produk yang belum terdaftar di sistem"
			/>
				...
				<div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3.5 flex items-start gap-3">
					...
					<div>
						<p className="text-xs font-semibold text-blue-800">
							Cara kerja request produk
						</p>
						<p className="text-xs text-blue-700 mt-0.5">
							Jika ada produk yang dikirim ke toko Anda tetapi belum terdaftar
							di sistem, ajukan request di sini. Tim sales akan memprosesnya dan
							Anda akan mendapat notifikasi di dashboard.
						</p>
					</div>
				</div>
			...
			<Modal
				...
				title="Request Produk ke Tim Sales"
				size="sm">
				<div className="space-y-3">
					<Input
						...
						hint={
							serialInput
								? `Disimpan sebagai: ${normalizeSerialNumber(serialInput)}`
								: undefined
						}
					/>
					<Input
					...
					<div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5">
						<p className="text-[11px] text-amber-700">
							Tim sales akan memverifikasi dan memasukkan produk ke sistem. Anda
							dapat memantau status di halaman ini.
						</p>
					</div>
