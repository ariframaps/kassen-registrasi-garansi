...
import { dealerProducts, mockDealerNotifications } from "@/mock/mock-data";
...
export default function DealerDashboardPage() {
	...
	const pending = dealerProducts.filter(
		(p) => p.warrantyStatus === "none",
	).length;
	const active = dealerProducts.filter(
		(p) => p.warrantyStatus === "active",
	).length;
	const expired = dealerProducts.filter(
		(p) => p.warrantyStatus === "expired",
	).length;

	const activeNotifs = mockDealerNotifications.filter(
							...
							<div
								...
								<div className="flex-1 min-w-0">
									<p className="text-xs font-semibold text-blue-900">
										{n.type === "product_ready"
											? "Produk Siap Diregistrasi"
											: "Peringatan Garansi"}
									</p>
									<p className="text-xs text-blue-700 mt-0.5">{n.message}</p>
									<div className="flex items-center gap-3 mt-2">
										<span className="font-mono text-[11px] text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md">
											{n.serialNumber}
										</span>
						...
						<div className="flex items-center gap-3">
							...
							<div>
								<p className="text-sm font-semibold text-white">
									{pending} produk belum diregistrasi garansinya
								</p>
								<p className="text-xs text-zinc-400 mt-0.5">
									Segera daftarkan agar garansi customer aktif
								</p>
							</div>
						</div>
						<Link href="/dealer/register-warranty">
					...
					<CardContent className="p-0">
								...
								{filtered.map((p) => {
									...
									return (
										<TableRow key={p.id}>
											...
											<TableCell>
												<span className="text-xs text-zinc-800 max-w-[160px] truncate block">
													{p.productType}
												</span>
											</TableCell>
											<TableCell>
											...
											<TableCell>
												{days !== null && p.warrantyStatus === "active" ? (
													<span
														className={`text-xs font-mono font-semibold ${days < 90 ? "text-amber-600" : "text-emerald-600"}`}>
														{days}h
													</span>
												) : (
						...
						<div className="px-5 py-3 border-t border-zinc-50">
							<p className="text-xs text-zinc-400">
								{filtered.length} dari {dealerProducts.length} produk
							</p>
						</div>
					</CardContent>
				</Card>
