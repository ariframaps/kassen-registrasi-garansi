...
interface ReassignModalProps {
  ...
  onClose: () => void;
  onSuccess: (productId: string, newDealerId: string, newDealerName: string) => void;
}

export function ReassignModal({ product, onClose, onSuccess }: ReassignModalProps) {
  ...
  const canReassign =
    product &&
    (product.status === "assigned_to_dealer" || product.status === "uploaded_by_sales");
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      success("Produk berhasil di-reassign", `SN ${product.serialNumber} → ${dealer?.name}`);
      onClose();
    } catch {
      ...
      {product && (
        <div className="space-y-4">
          ...
          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-100 space-y-1.5">
            ...
            <p className="text-sm font-mono font-semibold text-zinc-900">{product.serialNumber}</p>
            <p className="text-xs text-zinc-600">{product.productType} · {product.productCategory}</p>
            <div className="flex items-center gap-2 mt-2">
              <Badge variant="neutral">{product.assignedDealerName ?? "Tidak ada dealer"}</Badge>
              ...
              {selectedDealerId ? (
                ...
                <span className="text-xs text-zinc-400">Pilih dealer tujuan</span>
              )}
            </div>
          </div>
          ...
          {!canReassign && (
            <div className="flex gap-2 items-start p-3 bg-red-50 border border-red-100 rounded-xl">
              ...
              <p className="text-xs text-red-700">
                Produk ini sudah terjual (ada purchase aktif). Re-assign tidak tersedia.
              </p>
            </div>
          )}
