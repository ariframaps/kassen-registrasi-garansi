...
const validateCustomerSchema = z.object({
...
export async function POST(req: NextRequest) {
	try {
		...
		const parsed = validateCustomerSchema.parse(body);

		// Check if customer already exists
		const existingCustomer = await db.query.customer.findFirst({
			where: eq(customer.name, parsed.name),
		});

		if (existingCustomer) {
			throw new HttpError(
				`Customer '${parsed.name}' sudah terdaftar`,
		...
		if (parsed.email && parsed.email.trim()) {
			const existingEmail = await db.query.customer.findFirst({
				where: eq(customer.email, parsed.email.trim()),
			});
		...
		return NextResponse.json(
			successResponse({
				message: "Validasi berhasil - customer dapat dibuat",
				data: { isValid: true },
			}),
