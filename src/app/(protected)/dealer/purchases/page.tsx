...
function EditPurchaseModal({
	...
	const addableProducts = dealerProducts.filter(
		(p) =>
			...
			!selectedSNs.includes(p.serialNumber) &&
	...
	const removeSN = (sn: string) =>
		setSelectedSNs((prev) => prev.filter((s) => s !== sn));
	...
	const handleSave = async () => {
		...
		success(
			...
			`${selectedSNs.length} produk · ${form.customerName}`,
		);
	};
	...
	return (
		<>
			<Modal
				...
				description={`ID: ${group.id.toUpperCase()}`}
				size="xl">
				<div className="space-y-5">
					...
					<div>
						...
						<div className="space-y-3">
							...
							<Input
								label="Tanggal Pembelian (= Mulai Garansi)"
								type="date"
						...
						<div className="border border-zinc-100 rounded-xl overflow-hidden">
							{selectedSNs.map((sn) => {
								...
								return (
									<div
										key={sn}
										className={`flex items-center gap-3 px-4 py-2.5 border-b border-zinc-50 last:border-0 ${!isOriginal ? "bg-emerald-50" : ""}`}>
										...
										{!isOriginal && (
											<Badge variant="success">Baru ditambahkan</Badge>
										)}
										<button
											onClick={() => removeSN(sn)}
					...
					<div>
						<p className="text-xs font-semibold text-zinc-700 mb-2">
							Tambah Produk ke Pembelian Ini
						</p>
						<p className="text-[11px] text-zinc-400 mb-2">
							Hanya produk yang belum terdaftar garansi dan belum masuk
							pembelian lain
						</p>
						<Input
...
export default function DealerPurchasesPage() {
	...
	const [selectedDetail, setSelectedDetail] = useState<PurchaseGroup | null>(
		null,
	);
	...
	useEffect(() => {
		const loadPurchases = async () => {
			setLoading(true);
			try {
				const response = await dealerApi.getPurchases();
				if (!response.success) {
	...
	const getGroupStatus = (p: any): "active" | "expired" | "none" => {
		if (p.items.every((item: any) => item.warrantyStatus === "active"))
			return "active";
		if (p.items.every((item: any) => item.warrantyStatus === "expired"))
			return "expired";
		return "none";
	};
	...
	return (
		<div>
			<Topbar
				...
				description="Riwayat registrasi garansi per transaksi"
			/>
			<div className="p-6 animate-fade-up">
				...
				<div className="grid grid-cols-3 gap-4 mb-5">
					{[
						...
						{
							l: "Garansi Aktif",
							v: purchases.filter((p) => getGroupStatus(p) === "active").length,
							c: "text-emerald-600",
						},
						{
							l: "Garansi Berakhir",
							v: purchases.filter((p) => getGroupStatus(p) === "expired")
								.length,
							c: "text-red-600",
						},
					].map((s) => (
				...
				<Card>
					<div className="px-5 py-3.5 border-b border-zinc-100 flex flex-wrap gap-2.5 items-center">
						<div className="flex-1 min-w-48 max-w-64">
							<Input
								placeholder="Cari customer atau serial number…"
								value={search}
										...
										<tr>
											<td colSpan={7}>
												<EmptyState
													...
													description="Belum ada registrasi garansi"
												/>
											</td>
										</tr>
									) : (
													...
													<TableCell>
														<div>
															<p className="text-xs font-medium text-zinc-900">
																{p.customerProfile.name}
															</p>
															<p className="text-[11px] text-zinc-400">
																{p.customerProfile.phone}
															</p>
														</div>
													</TableCell>
													<TableCell>
															...
															<button
																onClick={() => {
																	const convertedGroup: PurchaseGroup = {
																		...
																		serialNumbers: p.items.map(
																			(item: any) => item.serialNumber,
																		),
																		invoiceFileName:
																			p.invoiceFile || "Tidak ada invoice",
																		invoiceUrl: "#",
			...
			<Modal
				...
				{selectedDetail &&
					(() => {
						const apiPurchase = purchases.find(
							(p) => p.id === selectedDetail.id,
						);
						const days = selectedDetail.warrantyEndDate
							? getDaysRemaining(selectedDetail.warrantyEndDate)
							: 0;
						return (
							<div className="space-y-4">
									...
									<div className="p-3 bg-zinc-50 rounded-xl space-y-1">
										<div>
											...
											<p className="text-xs font-medium text-zinc-800">
												{formatDateShort(selectedDetail.purchaseDate)}
											</p>
										</div>
										<div>
											<p className="text-[11px] text-zinc-400">Garansi s/d</p>
											<div className="flex items-center gap-2">
												<p className="text-xs font-medium text-zinc-800">
													{formatDateShort(selectedDetail.warrantyEndDate)}
												</p>
												{days > 0 ? (
													<Badge variant="success">{days}h</Badge>
												) : (
								...
								<div>
									<p className="text-xs font-semibold text-zinc-700 mb-2">
										{selectedDetail.serialNumbers.length} Produk
									</p>
									<div className="space-y-1.5">
										{apiPurchase?.items.map((item: any) => (
											<div
												key={item.productId}
												className="flex items-center gap-3 p-2.5 bg-zinc-50 border border-zinc-100 rounded-xl">
												<span className="font-mono text-xs bg-white border border-zinc-200 px-2 py-0.5 rounded-md text-zinc-700">
													{item.serialNumber}
												</span>
								...
								{/* Invoice */}
								{selectedDetail.invoiceFileName &&
									selectedDetail.invoiceFileName !== "Tidak ada invoice" && (
								...
								<div className="flex gap-2 pt-1">
									...
									<Button
										...
										onClick={() =>
											alert(
												"Edit functionality akan diimplementasikan di tahap berikutnya",
											)
										}>
										Edit Pembelian
									</Button>
